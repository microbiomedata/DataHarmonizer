...
Handsontable.cellTypes.registerCellType('dh.unit', {
...
class DataHarmonizer {
  ...
  getColumns() {
    ...
    for (let field of this.getFields()) {
      ...
      if (field.unit) {
        // Use most descriptive label available
        const label = field.unit.descriptive_name ||
          field.unit.ucum_code ||
          field.unit.symbol ||
          field.unit.abbreviation;
        // If a label is available, use dh.unit type
        if (label) {
          col.type = 'dh.unit';
          col.unit = label;
        }
